...
function drawHappyTIcon(size: number): Electron.NativeImage {
  ...
  // Violet gradient background inside rounded rect
  const cr = size * 0.25
  for (let y = 0; y < size; y++) {
    for (let x = 0; x < size; x++) {
      const nearL = x < cr, nearR = x > size - 1 - cr
      const nearT = y < cr, nearB = y > size - 1 - cr
      if ((nearL || nearR) && (nearT || nearB)) {
        const cx = nearL ? cr : size - 1 - cr
        const cy = nearT ? cr : size - 1 - cr
        if (Math.hypot(x - cx, y - cy) > cr) continue
      }
      const t = (x + y) / (2 * size)
      paint(x, y, (163 + (91  - 163) * t) | 0,
                  (148 + (66  - 148) * t) | 0,
                  (248 + (212 - 248) * t) | 0)
    }
  }
...
app.whenReady().then(() => {
  ...
  // System tray
  tray = new Tray(drawHappyTIcon(22))
